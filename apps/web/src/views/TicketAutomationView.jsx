			...
			{config.automationRules.map((rule, index) => (
				<section className="panel automation-rule" key={rule.id}>
					...
					<div className="automation-grid">
						...
						<label className="form-stack"><span className="form-label">{t('ticketAutomation.matchMode')}</span><select className="form-input" value={rule.matchMode} onChange={event => updateRule(rule.id, 'matchMode', event.target.value)}><option value="keywords">{t('ticketAutomation.keywords')}</option><option value="regex">{t('ticketAutomation.regex')}</option></select></label>
					</div>
					...
					<div className="automation-grid automation-actions-grid">
						<label className="form-stack"><span className="form-label">{t('ticketAutomation.category')}</span><select className="form-input" value={rule.categoryId ?? ''} onChange={event => updateRule(rule.id, 'categoryId', event.target.value)}><option value="">{t('ticketAutomation.keepCategory')}</option>{config.categories.map(category => <option key={category.id} value={category.id}>{category.label}</option>)}</select></label>
						<label className="form-stack"><span className="form-label">{t('ticketAutomation.priority')}</span><select className="form-input" value={rule.priority ?? ''} onChange={event => updateRule(rule.id, 'priority', event.target.value)}><option value="">{t('ticketAutomation.keepPriority')}</option><option value="low">{t('settings.priorityLow')}</option><option value="medium">{t('settings.priorityMedium')}</option><option value="high">{t('settings.priorityHigh')}</option></select></label>
