...
async function buildBackRow(guildId) {
	...
	return new ActionRowBuilder().addComponents(
		new ButtonBuilder().setCustomId(`${CONFIG_PREFIX}:home`).setLabel(backLabel).setEmoji(emojis.chevronleft).setStyle(ButtonStyle.Secondary),
	);
}
...
async function buildConfigHomeContainer(guildId, ticketConfig) {
	...
	const ticketCfgTitle = await t(guildId, 'ticket_cfg_title', { emoji: emojis.useredit });
	...
	const transcriptChanText = await t(guildId, 'ticket_cfg_transcript_chan', { channel: ticketConfig.transcriptChannelId ? `<#${ticketConfig.transcriptChannelId}>` : notSet });
	const autoCloseText = await t(guildId, 'ticket_cfg_auto_close', { minutes: ticketConfig.autoCloseMinutes || 0 });
	const requireReasonText = await t(guildId, 'ticket_cfg_require_reason', { enabled: ticketConfig.requireCloseReason ? '✓' : '✗' });
	const defaultPriorityText = await t(guildId, 'ticket_cfg_default_priority', { priority: ticketConfig.defaultPriority ?? 'medium' });
	...
	const navAdvanced = await t(guildId, 'ticket_cfg_nav_advanced');
	const navAdvancedDesc = await t(guildId, 'ticket_cfg_nav_advanced_desc');
		...
		.addActionRowComponents(
			new ActionRowBuilder().addComponents(
				new StringSelectMenuBuilder()
					...
					.addOptions(
						{ label: navChannels, description: navChannelsDesc, value: 'channels', emoji: emojis.folder },
						{ label: navPanel, description: navPanelDesc, value: 'panel', emoji: emojis.monitorcog },
						{ label: navCategories, description: navCategoriesDesc, value: 'categories', emoji: emojis.folders },
						{ label: navRoles, description: navRolesDesc, value: 'roles', emoji: emojis.shield },
						{ label: navLimit, description: navLimitDesc, value: 'limit', emoji: emojis.infinity },
						{ label: navAdvanced, description: navAdvancedDesc, value: 'advanced', emoji: emojis.settings },
					),
			),
		);
}
...
async function buildConfigChannelsContainer(guildId, ticketConfig) {
	...
	const transcriptDesc = await t(guildId, 'ticket_cfg_transcript_desc');
	...
	const selectTranscriptPlaceholder = await t(guildId, 'ticket_cfg_select_transcript_placeholder');
	...
	return new ContainerBuilder()
		...
		.addActionRowComponents(
			new ActionRowBuilder().addComponents(
				new ChannelSelectMenuBuilder()
					.setCustomId(`${CONFIG_PREFIX}:channels:transcript`)
					.setPlaceholder(selectTranscriptPlaceholder)
					.addChannelTypes(ChannelType.GuildText),
			),
		)
		.addSeparatorComponents(s => s.setSpacing(SeparatorSpacingSize.Small).setDivider(false))
...
async function buildConfigRolesContainer(guildId, ticketConfig) {
	...
	const supportRolesSelectDescription = await t(guildId, 'ticket_cfg_support_roles_select_desc');

	const select = new RoleSelectMenuBuilder()
		.setCustomId(`${CONFIG_PREFIX}:roles:set`)
		.setPlaceholder(supportRolesPlaceholder)
		.setMinValues(0)
		.setMaxValues(10);
...
async function buildConfigAdvancedContainer(guildId, ticketConfig) {
	...
	const autoCloseDesc = await t(guildId, 'ticket_cfg_auto_close_desc');
	const requireReasonDesc = await t(guildId, 'ticket_cfg_require_reason_desc');
	const defaultPriorityDesc = await t(guildId, 'ticket_cfg_default_priority_desc');

	const autoCloseSelect = new StringSelectMenuBuilder()
		.setCustomId(`${CONFIG_PREFIX}:advanced:auto_close`)
		.setPlaceholder(`Current: ${ticketConfig.autoCloseMinutes || 0} min`)
		.addOptions([0, 30, 60, 120, 240, 1440].map(min => ({
			label: min === 0 ? 'Disabled' : `${min} minutes`,
			value: String(min),
			default: min === ticketConfig.autoCloseMinutes,
		})));

	const requireReasonSelect = new StringSelectMenuBuilder()
		.setCustomId(`${CONFIG_PREFIX}:advanced:require_reason`)
		.setPlaceholder(`Current: ${ticketConfig.requireCloseReason ? 'On' : 'Off'}`)
		.addOptions([
			{ label: 'Off', value: 'false', default: !ticketConfig.requireCloseReason },
			{ label: 'On', value: 'true', default: ticketConfig.requireCloseReason },
		]);

	const prioritySelect = new StringSelectMenuBuilder()
		.setCustomId(`${CONFIG_PREFIX}:advanced:default_priority`)
		.setPlaceholder(`Current: ${ticketConfig.defaultPriority ?? 'medium'}`)
		.addOptions(['low', 'medium', 'high'].map(p => ({
			label: p,
			value: p,
			default: p === (ticketConfig.defaultPriority ?? 'medium'),
		})));
	...
	return new ContainerBuilder()
		...
		.addActionRowComponents(new ActionRowBuilder().addComponents(autoCloseSelect))
		.addTextDisplayComponents(tMsg => tMsg.setContent(requireReasonDesc))
		.addActionRowComponents(new ActionRowBuilder().addComponents(requireReasonSelect))
		.addTextDisplayComponents(tMsg => tMsg.setContent(defaultPriorityDesc))
...
async function renderConfigPage(page, guildId, ticketConfig) {
	...
	case 'advanced': return buildConfigAdvancedContainer(guildId, ticketConfig);
...
async function handleCategoryModal(interaction, ticketConfig, slot) {
	...
	categories[slot - 1] = { id: customId, label, emoji, description, priority: existing?.priority ?? null, supportRoleIds: existing?.supportRoleIds ?? [] };
...
async function handleConfigComponent(interaction, ticketConfig) {
	...
	case 'channels': {
		...
		if (action === 'thread') {
			...
			await updateTicketConfig(interaction.guildId, { transcriptChannelId: channelId });
			ticketConfig.transcriptChannelId = channelId;
		}
	...
	case 'panel': {
		...
		if (action === 'refresh') {
			...
			try {
				...
				await interaction.followUp({
					components: [buildTextContainer(`${checkEmoji} Panel refreshed.`)],
					flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
				});
			}
			catch (error) {
		...
		if (action === 'send') {
			...
			try {
				...
				await interaction.editReply({ components: [panelContainer] });
				await interaction.followUp({
					components: [buildTextContainer(`${checkEmoji} Panel sent to <#${channelId}>.`)],
					flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
				});
			}
			catch (error) {
	...
	case 'advanced': {
		if (action === 'auto_close') {
			...
			await updateTicketConfig(interaction.guildId, { autoCloseMinutes: minutes });
			ticketConfig.autoCloseMinutes = minutes;
		}
		else if (action === 'require_reason') {
			const enabled = interaction.values[0] === 'true';
			await updateTicketConfig(interaction.guildId, { requireCloseReason: enabled });
			ticketConfig.requireCloseReason = enabled;
		}
		else if (action === 'default_priority') {
			const priority = interaction.values[0];
			await updateTicketConfig(interaction.guildId, { defaultPriority: priority });
			ticketConfig.defaultPriority = priority;
		}
		const advancedContainer = await buildConfigAdvancedContainer(interaction.guildId, ticketConfig);
		await interaction.update({ components: [advancedContainer] });
		return;
	}
